...
export default function PatientViewPage() {
  ...
  // Load incidents filtered by current patient
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-gray-100">
      ...
      {/* Top navbar */}
      <header className="flex justify-between items-center p-4 bg-gray-800/80 backdrop-blur-md shadow-lg">
        <div className="flex items-center gap-2">
          <img
            ...
            alt="Logo"
            className="w-12 h-12 rounded-full border-4 border-blue-400 shadow mb-1"
          />
          <h1 className="font-bold text-xl text-blue-400">ENTNT Dental</h1>
        </div>
        <button
          onClick={handleLogout}
          className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm shadow transition"
        >
          Logout
        </button>
      </header>
      ...
      {/* Welcome section */}
      <section className="flex flex-col items-center mt-8 mb-6">
        <img
      ...
      {/* Stats cards */}
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto mb-6 px-4">
        <StatCard title="Total Visits" count={incidents.length} color="text-green-400" ring="ring-green-400/50" />
        <StatCard title="Upcoming" count={upcomingCount} color="text-yellow-300" ring="ring-yellow-400/50" />
        <StatCard title="Completed" count={completedCount} color="text-purple-300" ring="ring-purple-400/50" />
      </section>
      ...
      {/* Incident history */}
      <section className="max-w-5xl mx-auto space-y-4 px-4 pb-6">
        {incidents.length === 0 ? (
          ...
          incidents.map(i => (
            <div
              key={i.id}
              className="bg-gray-700/60 backdrop-blur-md rounded-xl p-4 shadow hover:shadow-xl hover:ring-2 hover:ring-blue-400/40 transition"
            >
              <div className="flex justify-between items-center mb-2">
                <h3 className="text-lg font-semibold text-blue-400">{i.title}</h3>
                <span className="text-sm text-gray-400">{new Date(i.appointmentDate).toLocaleDateString()}</span>
              </div>
...
function StatCard({ title, count, color, ring }) {
  return (
    <div className={`bg-gray-700/60 backdrop-blur-md rounded-xl p-4 shadow hover:shadow-xl hover:ring-2 ${ring} transition text-center`}>
